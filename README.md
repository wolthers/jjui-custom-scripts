...
| `pr checkout [pr-number]`    | Check out a PR in a new jj workspace: fetch, create workspace (`.env*`, direnv, registry), `jj checkout <branch>@origin`, open in Cursor. Use `--change-id` to resolve from change; `--no-open` to skip opening Cursor. Prompts for `pr-number` if omitted and TTY. |
...
| Command                                                                      | Description                                                                                                                                                                                  |
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `workspace add [destination] [--name <name>] [--prefix <prefix>] [-r <rev>]` | Create workspace sibling to repo root, copy `.env*`, run `direnv allow`. Prompts for destination if omitted and TTY. Use `ai-implement` for agent-backed workspace.                          |
...
| `workspace list [--registry-only]`                                           | List workspaces (`jj workspace list` by default). Use `--registry-only` for one name per line from registry. Prunes stale entries before listing. Exits with message when registry is empty. |
